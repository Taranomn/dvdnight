...
export async function getCommonWatchlist(userId: string, friendId: string) {
  ...
  const theirMovieIds = new Set(theirs.map((item) => item.movie_id));
  return mine
    .filter((item) => theirMovieIds.has(item.movie_id))
...
export async function getGroupCommonWatchlist(groupId: string) {
  ...
  const [first = []] = lists;
  const otherSets = lists.slice(1).map((list) => new Set(list.map((item) => item.movie_id)));
  const movies = first
    .filter((item) => otherSets.every((set) => set.has(item.movie_id)))
    .map((item) => item.movies)
    .sort((a, b) => a.title.localeCompare(b.title));
