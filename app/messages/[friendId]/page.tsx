...
export default async function MessageThreadPage({ params }: { params: Promise<{ friendId: string }> }) {
  ...
  return (
    <div className="mx-auto flex h-[calc(100dvh-5.5rem)] max-w-5xl flex-col overflow-hidden rounded-[2rem] border border-white/10 bg-[#05050a]/70 shadow-2xl shadow-black/40 md:h-[calc(100dvh-3rem)]">
      <header className="flex items-center justify-between gap-3 border-b border-white/10 bg-[#05050a]/95 px-4 py-4 backdrop-blur-2xl">
        ...
        <div className="flex shrink-0 items-center gap-3 text-[#ff3b5c]">
          <button className="rounded-full p-2 transition hover:bg-white/[0.06]" aria-label="Voice call">
            <Phone className="h-6 w-6" />
          </button>
          <button className="rounded-full p-2 transition hover:bg-white/[0.06]" aria-label="Video call">
      ...
      <main className="flex-1 overflow-y-auto bg-[radial-gradient(circle_at_top_right,rgba(255,59,92,0.12),transparent_28rem),#05050a] px-4 py-6">
        <div className="mx-auto mb-6 w-fit rounded-full border border-white/10 bg-white/[0.055] px-5 py-2 text-sm text-zinc-400">
          Today
        </div>
        <div className="space-y-5">
          ...
          {!messages.length ? (
            <div className="mx-auto max-w-sm rounded-[2rem] border border-white/10 bg-white/[0.045] p-6 text-center text-zinc-400">
